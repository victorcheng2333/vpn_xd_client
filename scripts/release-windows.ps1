...
[xml]$project=Get-Content Apps/Windows/XDVPN.App/XDVPN.App.csproj
$version=[string]$project.Project.PropertyGroup.Version
if($version -notmatch '^(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$' -or $Tag -cne "windows-v$version"){throw 'Windows tag must match the App project version (windows-vMAJOR.MINOR.PATCH).'}
...
if($Action -eq 'prepare') {
    ...
    foreach($binary in @((Join-Path $root $names[0]),"$root/windows-x64/XDVPN.App.exe","$root/windows-x64/XDVPN.Service.exe")){
        if((Get-Item $binary).VersionInfo.FileVersion -ne "$version.0"){throw "Embedded version mismatch: $binary"}
    }
...
for($attempt=0;$attempt -lt 12;$attempt++){
    $remote=Gh-Value @('api',"repos/$Repository/releases/tags/$Tag") | ConvertFrom-Json
