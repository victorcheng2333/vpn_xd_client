...
- 下载后可用 `shasum -a 256 -c <DMG 文件名>.sha256` 校验完整性。
- 升级前断开 VPN 并退出旧版，将 DMG 中的应用拖入 Applications；按应用提示升级系统助手。
- 配置和钥匙串密码沿用；不会自动替换运行中的应用或系统助手。
- 私有发布仓库需要 GitHub 访问权限；客户端更新设置支持本机钥匙串保存只读 Token。
- third-party-sources.tar.gz 提供对应第三方源码，构建脚本见本版本源码的 scripts/build-openconnect.sh。
