...
- 源码提交 `a441959f136d22ed3dc5baf00703c5ff0bcbfb65` 已通过双架构原生 CI `34101717340`：ARM 与 Intel 均完整构建并通过 **222 项 Swift 测试、0 失败**，包括可控计时器的重复事件回归；发布规则 14 项及打包保护也通过。记录 `.build/release-timer-ci-success.log`。
- 最终版本提交仅更新版本号、构建号和文档；正式流程继续在该提交重建、重跑回归、公司签名、App／DMG 公证及票据与来源复核后发布。正式文件还需从 GitHub 下载后独立复核，并确认匿名更新接口返回此版本。

## 2026-09-07：恢复期限回归改为可控计时验证

- 双架构 CI `34100467843` 的 ARM 完全通过，Intel 仅“重复事件不能延后期限”用例失败。该用例以 120 + 140 毫秒睡眠推测后台计时任务已经执行，仍受云端调度顺序影响。
- 为恢复期限增加可注入的异步等待函数，默认仍为原有 `Task.sleep(for:)`，正式版期限仍为 3 秒。该用例通过可控等待器确认整次恢复只创建一个计时任务、重复 DPD／网络事件不重新计时，释放等待后必须进入断开清理；不再依赖几十毫秒的调度余量。其他期限、取消与实际时间检查保留。
- 最终本地 ARM／Intel（Rosetta）Release 配置完整回归各 **222 项通过、0 失败**，记录 `.build/release-timer-{arm,intel}-tests.log`。本次只增加测试注入入口，默认网络恢复行为不变；未替换已安装 App 或操作当前 VPN。后续正式版本仍需双架构原生 CI 与正式打包流程通过。

## 2026-09-07：发布前统一修正异步回归等待，并加入双架构 CI

- `v1.1.21` 发布运行 `34098549750` 中，Intel 原生构建、测试、公证通过；ARM 的未变化网络通知用例在真实 SSID 通知后仅等待 50 毫秒，断言先于恢复命令执行，发布再次被拦截。该标签未发布正式 Release。
- 网络状态用例改为等待实际命令或状态，默认把无关的恢复超时放在等待预算之外；12 个专门验证期限／取消的用例显式保留 200 毫秒测试期限。没有调整生产代码、生产超时或移除原有断言；等待失败会输出状态和命令种类，便于定位。重复通知的禁止重连观察窗口仍保留。
- 本机复验期间，系统日志证实多次 Maintenance Sleep／DarkWake，测试进程被暂停，额外触发了网络替身及恢复期限超时。临时保持系统唤醒后，最终 ARM 和 Intel（Rosetta）Release 配置完整回归各 **222 项通过、0 失败**。记录 `.build/release-preflight-{arm,intel}-active-tests.log`；发布规则 14 项、打包保护检查通过。未改变永久电源设置或当前 App／VPN。
- 日常 CI 扩为 macos-15 ARM 和 macos-15-intel 两个原生 runner；先验证源码提交，再创建新的正式版本标签。发布流程继续要求每个架构的原生测试、公司签名和 Apple 公证全部通过。

## 2026-09-07：1.1.21 修正发布测试时序并重新验证

- `v1.1.20` 的 GitHub 发布运行 `34097240047` 被 Intel 原生回归拦截：222 项测试中，冷却时间用例的两条断言失败；ARM 构建、222 项测试和 App／DMG 公证通过，发布阶段未执行。保留原标签，不替换已推送的源码历史；下一次正式版本为 **1.1.21 / build 30**。
- 该用例用固定 50 毫秒等待首条恢复命令，却同时启用 200 毫秒恢复超时，存在慢速调度下先超时或提前模拟回复的竞争。改为等待实际首条命令并检查状态，将此用例不验证的恢复超时独立设为 10 秒；两次请求、150 毫秒最短间隔及连续通知合并断言保留。应用运行逻辑没有改变，其他恢复超时测试仍使用原期限。
- 本地 Release 配置完整回归：ARM **222 项通过、0 失败**，Intel（Rosetta）**222 项通过、0 失败**。修正用例两种架构各独立运行 **12 次全部通过**；发布规则 14 项及打包保护检查通过。记录 `.build/release121-{arm,intel}-tests.log`、`.build/release121-cooldown-*.log`。Intel 测试显式指定 `--triple x86_64-apple-macosx14.0`，避免 SwiftPM 复用 ARM 构建。
- 真实网络与系统服务验收沿用下节同一运行代码 `1257f98` 的结果及限制。当前运行中的 App、系统服务和 VPN 保持不动；正式发布仍须通过 GitHub 两种架构的构建、测试、公证及产物复核。

## 2026-09-07：1.1.20 发布前实网验收

- 实网使用已安装、已公证的 `1.1.19-test.28`（运行代码 `1257f98`）；本轮正式版本改为 **1.1.20 / build 29**，仅更新版本与发布说明，运行逻辑相同。ARM／Intel 的 222 项回归此前均通过，发布流程还会在正式构建配置重跑。发布规则 14 项和打包保护预检通过。
- 真实 VPN 认证、隧道与公司指定只读页面验收通过：HTTPS 返回 200、证书验证成功、路由经 VPN 接口；复用已有浏览器登录态，实际应用列表正常加载。没有读取或输出 VPN 密码，也没有向验收页面提交数据。
- 手动断开后，原 VPN 接口对应的 DNS 与路由消失、OpenConnect 引擎退出、普通 HTTPS 上网正常；自动连接偏好保持开启，但手动断开没有自行重连。再次连接及内网访问成功。App 保留空控制会话以便再次连接，此时服务 busy 不等于引擎仍在运行。
- 实际关闭 Wi-Fi 约 20 秒后恢复。按照现有策略，离线期间保留已建立会话并暂停主动恢复；物理网络重新就绪后约 **1.4 秒**恢复 VPN，指定 HTTPS 页面再次返回 200。实际出口网关／本机 IP 变化时，服务器路由重建与读回验证成功。上述时间为本次样本，不是性能保证。
- 用户配合短暂睡眠／唤醒；应用收到真实系统 sleep 和 wake 通知，唤醒后约 **1.25 秒**恢复 VPN，内网页面再次返回 200。没有进行长时间睡眠或休眠验收。
- 主动退出 App 后，检查受到用户随后重新打开并建立新连接的干扰：旧 OpenConnect PID 已结束，新 App 可成功获得会话并连接。原检查读到了新引擎，不能据此判定旧进程清理失败，也不能用这次采样声称测得退出清理耗时。后续保持用户当前连接，不再次主动退出应用。
- 实网原始诊断仅保留在本机 `.build/release120-network/`，不提交内部页面内容、账号、网络地址或浏览器会话数据。
...
- 覆盖 Applications 中的 test.26 后，旧后台 PID 14008 继续运行。新版 XPC 客户端日志明确记录 `Received message forbidden due to code signing requirement`；「重新注册」此前要求先取得旧服务状态，因此未进入正常注销。没有放宽签名要求或用未经认证的回复判断服务状态。
- 修正为仅在用户显式重新注册时使用 SMAppService 管理的停止、等待退出和再注册流程。可读取的忙碌状态仍阻止替换；未知状态的修复会停止旧服务，页面提示先断开其他版本；自动连接和状态刷新不触发停止。新增回归覆盖等待退出、忙碌拒绝、取消、停止失败，与之前的短暂状态同步重试一起验证。
