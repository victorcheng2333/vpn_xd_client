...
- 服务启动时复制完整 App 到 `/private/var/run` 下随机创建的 root 私有目录，再校验副本公司签名及其与源 App 的构建／引擎指纹。引擎与清理脚本只从此副本执行；副本拒绝链接、移除其他用户权限及 set-id 位。源 App 被替换会阻止新的连接，但不会改变活动隧道的清理程序。正常退出清理副本；异常强杀可能留下仅 root 可访问的副本，不能在未确认无活动子进程时清除。
