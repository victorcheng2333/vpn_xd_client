...
def preflight(client, config):
    ...
    wanted = config.get('external_group_id')
    matches = [g for g in groups if g['id'] == wanted and not g['attributes']['isInternalGroup']]
    if len(matches) != 1:
        raise ValueError('Configure external_group_id with an existing external group for this app.')
    return matches[0]
...
def compliance(client, config, row):
    ...
    if state in ('MISSING_EXPORT_COMPLIANCE', 'IN_EXPORT_COMPLIANCE_REVIEW'):
...
def finish(client, config, row, group, notes, locale, submit_review, notify_testers=False):
...
def main():
    ...
    with lock_path.open('w') as lock:
        ...
        print(f'Target: iOS {args.version} ({build}), external group {group["attributes"]["name"]}', flush=True)
        if args.action == 'plan':
            review_metadata(client, config['app_id'])
            print('API/app/group/review metadata verified. Encryption configured: ' +
        ...
        if args.action == 'release':
            ...
            values = ['--version', args.version, '--build', build, '--audience', 'external']
            ...
            (output / 'upload-attempt.json').write_text(json.dumps({'version': args.version, 'build': build}))
            subprocess.run(cmd + ['upload'] + values, env=env, check=True)
        row = client.wait_build(config['app_id'], args.version, build, args.timeout)
        state = finish(client, config, row, group, notes, args.locale, args.submit_review, args.notify_testers)
        ...
        result = {'version': args.version, 'build': build, 'build_id': row['id'],
                  'external_state': state, 'testable': state == 'IN_BETA_TESTING',
        ...
        if state in ('BETA_REJECTED', 'EXPIRED', 'PROCESSING_EXCEPTION', 'NOT_APPLICABLE', 'MISSING_EXPORT_COMPLIANCE'):
            raise RuntimeError('External testing requires attention: ' + state)
        if state != 'IN_BETA_TESTING':
            print('Not yet externally testable. Query status; Apple review may still be required.')
