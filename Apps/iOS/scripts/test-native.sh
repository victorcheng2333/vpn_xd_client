...
set -euo pipefail
cd "$(dirname "$0")/.."
[ -n "${1:-}" ] || { echo 'Usage: test-native.sh SIMULATOR_UDID [untrusted-local-https-url] [trusted-https-url]' >&2; exit 2; }
mkdir -p .build/tests
...
xcrun simctl spawn "$1" "$PWD/.build/tests/engine-smoke" "${2:-}" "${3:-}"
