...
## 23:24 起网络切换与唤醒重连排查（本地时间 UTC+8）

- 从 iPhone 17 Pro 导出完整质量历史，15:27:17Z 基线共有恢复成功 286 次，其中 267 次原因是 `wake`，16 次 `transport`、1 次 `network`、2 次 `providerRestart`。日志中的停止原因 1 是系统的 `userInitiated`。统计成功只代表隧道恢复，不等于每次业务访问通过。
- 旧实现的每次 `wake()` 都主动发送 PAUSE，拆除健康 TLS。修复后唤醒仅发送唤起引擎的统计命令，让引擎检查链路定时器；显式启用 30 秒 DPD，实际断线才记录恢复。旧质量历史保留，不重写已有次数。
- 原生引擎在认证/网络设置回调期间尚未进入 mainloop，旧 `updateNetworkAvailable` 会丢失重连请求。现在保存待处理请求并合并 PAUSE；保持单 worker 操作 C 会话。Provider 复用相同网络设置，避免每次 TLS 恢复都重装路由；路径签名按使用中的物理接口过滤并保留偏好顺序，日志增加接口名。
- 新增回环 AnyConnect 测试模式，在设置回调内注入断网/恢复。修改前引擎在 8 秒 watchdog 前不能恢复，同一测试在修复后通过：一次合并重连、真实 CSTP 双向包回送、连续十次健康唤醒不增加 CONNECT。另有原生 CONNECT 401 分类/取消/表单回归和 102 项现有逻辑检查通过。测试执行于 iOS 模拟器，不替代真机换网。
- 本工作区 iPhoneOS 签名构建及 `codesign --verify --deep --strict` 通过，App/Extension 描述文件有效至 2027-09-07，团队与权限匹配。构建时工作区另有本地网络例外修改；此处不把该改动列为重连修复。
- **安装流程失误与未完成项：** 用户回复已断开后，15:32:09Z 的安装前导出仍为已连接且自动恢复启用。执行者未在发出安装前阻断失败的状态断言，15:32:29Z 覆盖安装完成。两次只读启动均被 iOS Security 校验拒绝，用户反馈“出现无法验证或不能上网”。已停止安装/连接指令，指导关闭系统按需连接并断开，必要时删除系统 VPN 配置并重启；尚待确认网络恢复和新版启动。不能将本次安装记为真机启动或切网验收通过。
- 本机忽略证据目录：当前仓库 `Apps/iOS/.build/reconnect-validation/`，含 `baseline.json`、`preinstall.json`、`native-before.log`、`native-recovery.log`、`native-expiry.log`、`device-build.log`、`install.json` 和两次 `installed-launch*.json`。

### 23:38 起重启后继续验收

- 用户确认重启后已解锁、手机网络正常。CoreDevice 最新安装列表确认 XD VPN 已卸载，随后于 23:40:22 新装已通过签名校验的构建，23:40:43 启动成功。
- 核对另一个任务的改动：`VPNModel.swift` 的局域网例外修改早于本次构建，已包含在同一真机包内；macOS 的脚本执行优化不编入 iOS。没有覆盖其他任务的修改。
- 23:41:05 的新鲜导出为尚未连接、系统自动恢复关闭、包计数及质量历史为零。没有通过调试参数触发 VPN 连接。用户重新保存账号、开启自动连接偏好并手动连接，确认“已连接，内网访问正常”。
- 用户将 App 留在后台执行 Wi-Fi → 蜂窝 → Wi-Fi，两次均确认内网业务正常。15:42:27.108Z–15:42:28.484Z 网络恢复耗时 1.376 秒，物理接口变为 `pdp_ip0`；15:42:55.119Z–15:42:55.223Z 恢复耗时 0.104 秒，接口回到 `en0`。两次都没有重复下发相同的网络设置，未发生重新认证。这些耗时是客户端观测到的恢复过程，不是从用户点击 Wi-Fi 开关起算的完整业务中断时长。
- 用户继续执行两次约 20 秒短锁屏，确认解锁后内网均正常；期间没有额外恢复记录。日志未出现 provider 的 sleep/wake 回调，因此该结果证明短锁屏业务可用，不能替代真正系统睡眠、长时间锁屏或 DPD 失活测试。
- 15:44:55.625Z 手动断开；用户确认普通上网正常、没有自行重连。15:45:19.817Z 有新的手动连接意图，用户明确确认测试完又主动点击连接，不能将其判为自动连接失控。
- 后台切网/锁屏测试期间未发出主 App 启动命令。15:45:31 开始最终只读导出；15:45:44 有一次 provider 重启，与调试启动相邻，单独标记为采集期间事件，不计入上述受控切网验收。共享目录复制仍遇 CoreDevice 路径错误，不能直接后台读取共享状态。
- 15:46:02Z 最终新鲜导出为已连接、TLS、系统自动恢复启用，上行 41 / 下行 39 包。保留用户主动重新连接后的状态。未额外执行“保持断开后重新启动 App”的独立验收，因为用户已在最终采集前主动重连。
- 本轮通过：新装与真实启动、手动连接及内网业务、两个方向的后台 Wi-Fi/蜂窝切换、两次短锁屏业务访问、手动断开后上网正常。长锁屏、隔夜、飞行模式、持续快速切网和个人热点兼容性仍不在本轮通过范围。
- 新增证据：`resumed-apps.json`、`resumed-install.json`、`resumed-installed-launch.json`、`resumed-idle.json`。
- 最终证据：`final-launch.json`、`final-state.json`、`final-state-late.json`。

### 23:48 / 23:55 个人热点失败复测

- 用户反馈手机自身 VPN 正常，但 Mac 连接手机热点后无网络，切回其他 Wi-Fi 后恢复。此场景不能算此前 Wi-Fi/蜂窝双向切换验收的一部分，也不能据手机自身联网正常判定热点修复成功。
- Mac configd 两次记录 `DHCP en0: INIT-REBOOT (172.20.10.3) timed out`、`server not responding`，分别于 23:48:22.828 和 23:55:29.822 撤销 Wi-Fi 地址；USB 热点 en13 同样出现 DHCP 超时。最初显示的地址来自已有租约，不能当作收到本次 DHCP ACK 的证据。Mac VPN 已更新网关主机路由至 172.20.10.1，但随后因缺少 IPv4 网络反复报 `Network is unreachable`；旧隧道清理后 DHCP 仍无回应。
- 23:56:12 Mac 切到另一 Wi-Fi 获得 IPv4，23:56:14.181 VPN 建立成功。上述日志定位了地址配置失败这一直接原因，尚未证明 DHCP 请求或响应具体在手机的哪个环节被阻断。
- 当前源码/真机构建包含 `excludeLocalNetworks=true`，用户在新装后重新保存过配置；热点仍失败，因此恢复本地网络例外尚不足以证明解决此兼容问题。没有因本次排查关闭 `includeAllNetworks`。
- 手机 Documents 中的自动导出仍停在 23:46:02，不能用来确认后续热点测试时的扩展状态。App Group 复制失败，调试器读取受锁屏/附加限制未取得新鲜数据；已请求用户分享手机内最新报告。未在这两次热点复测后通过重启主 App 采集。
- 证据位于 `Apps/iOS/.build/reconnect-validation/`：`hotspot-configd.log`、`hotspot-mac-latest.jsonl`、`hotspot-export.json`。

### 00:01:48 手机分享报告补证（2026-09-08）

- 用户提供手机内新鲜报告，事件时间 2026-09-07 16:01:48Z：系统已连接、TLS，上行 2645 / 下行 2508 / 丢弃 64；恢复成功 7、失败 0、取消 0，自动恢复启用。这是用户在手机分享的报告，不是先前 23:46 的调试导出。
- 与 Mac 第一轮对齐：手机 23:48:13 路径切到 `pdp_ip0`，同秒恢复已连接；Mac 23:48:22.828 DHCP 确认超时并撤销 172.20.10.3。
- 第二轮：手机 23:55:21 切到 `pdp_ip0`，同秒恢复已连接；Mac 23:55:29.822 DHCP 确认超时并撤销 172.20.10.3，23:55:38.074 USB 热点地址也因 DHCP 超时被撤销。手机报告在这段时间没有恢复失败或隧道停止，直到 23:57:38 切回 `en0` 后同秒恢复。
- 手机另有 23:52:34 用户停止、23:52:36 启动、23:52:37 连接成功记录；这段约两秒断开没有配套的热点 DHCP 对照结果，不足以证明关闭手机 VPN 后热点立即恢复。
- 当前结论：热点失败发生时手机隧道报告已恢复，Mac 失败的直接证据是 DHCP 无应答和 IPv4 地址撤销。仍无法仅凭报告判断 DHCP 请求/响应是否进入 PacketTunnel，或热点转发流量是否进入手机 VPN。64 个丢弃包含 IPv6 策略阻断，不能当作 DHCP 丢包证据。`excludeLocalNetworks=true` 候选修复未通过热点验收；本轮不改变 `includeAllNetworks`。

### 个人热点根因与修复（2026-09-08 00:20 起，真机验收通过）

- 对齐三方证据：Mac App 日志 15:15:36Z 起等待热点 DHCP，手机报告 15:16:01Z 记录 `系统停止隧道（1）`（用户手动关闭手机 VPN），Mac configd 15:16:02.43Z 即取得 172.20.10.3 并于 15:16:03.96Z 通过热点恢复 VPN；此前 23:48／23:55 两轮在 `excludeLocalNetworks=true` 构建上仍是 Wi-Fi 与 USB 热点 DHCP 超时。结论：阻断来自 `includeAllNetworks` 的系统级强制，本地网络例外不覆盖热点的 DHCP 服务应答。
- 修复：`VPNModel.swift` 保存配置时 `includeAllNetworks=false`，全隧道只依赖网关 0.0.0.0/0 路由（IPv6 阻断路由保留）。已知代价：隧道重建期间系统不再拦截非隧道流量。`excludeLocalNetworks=true` 保留但此时无系统效果。102 项逻辑检查与无签名模拟器构建通过。
- 安装：00:17 本地签名构建（Apple Development: Tools UG，描述文件 com.xd.vpn.ios.poc / .PacketTunnel）。安装前只读导出确认隧道已由用户于 16:17:58Z 停止、系统自动连接关闭；`devicectl` 安装 `success`（新 bundle `BF737AE2…`），随后只读启动通过 iOS 校验（pid 925），未出现“无法验证”。证据：`Apps/iOS/.build/hotspot-fix/{device-build.log,precheck-state.json,install.json,postinstall-state.json,verify-state.json}`。
- 验收：用户在手机重新保存配置并连接（16:19:35Z、16:19:51Z 各一次认证成功），16:20:05Z 开启热点时手机路径切到 `pdp_ip0`，同秒恢复已连接；16:22:30Z 只读导出为已连接、TLS、上行 10242／下行 11294、最近 24 小时恢复成功 9 次／失败 0 次／取消 0 次。Mac 侧 16:20:05.97Z 断开原 Wi-Fi，16:20:06.55Z 即经热点取得 172.20.10.3（此前需 42 秒且超时），en0 与 en13（USB 共享）同时得到地址，16:20:08.38Z Mac VPN 经 172.20.10.1 恢复；16:21:06Z 切回 Wi-Fi 0.55 秒恢复。用户确认 iOS 端无异常、Mac 经热点重连顺畅。
- 未覆盖：热点客户端流量是否进入手机隧道未单独验证（Mac 自身 VPN 经热点可建立，说明至少网关可达）；长锁屏、隔夜和持续快速切网仍待验收。
...
- 真机（2026-09-08 16:17，iPhone 17 Pro，Tools UG 自动签名 Debug 构建，`main@65c6f6d`）：安装前 16:16:31 只读导出为「尚未连接」；用户于 16:16:54 在旧包上手动连接，16:17:35 覆盖安装时隧道处于活动状态，系统随即重启扩展，新扩展 16:17:36 在 0.4 秒内以已保存凭据重新认证并连接（TLS，IPv4 全隧道，IPv6 阻断），只读启动通过 iOS 校验。`includeAllNetworks` 关闭后，在连接中覆盖安装未再出现断网现象，但仍建议按说明先断开。冷却路径需网关连续拒绝新会话才会触发，本轮未能在真机复现；证据在本机忽略目录 `Apps/iOS/.build/cooldown-validation/`。
- 工程配置：`Base.xcconfig` 现在直接包含 `DEVELOPMENT_TEAM = KQY8A3BNVG`，不再依赖每台机器的 `Signing.local.xcconfig`（该文件仍可用于覆盖团队）。
