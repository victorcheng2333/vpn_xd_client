...
默认仅发布到内测组「iOS 真机验证」，导出时启用 `testFlightInternalTestingOnly`。内测不提交 Beta Review，也不创建或提交 App Store 正式版本。只有明确要求外测时才使用 `--audience external`。
...
在被 Git 忽略的 `TestFlight.local.json` 填写 Key ID、Issuer ID、仓库外 `.p8` 的路径。内测组 ID 已固定在 `TestFlight.json`，可用 `internal_group_id` 或 `ASC_INTERNAL_BETA_GROUP_ID` 覆盖；外测才需要 `external_group_id`。
...
只有外测需要先在 Connect 完成测试描述、反馈邮箱、审核联系人和必要的审核专用 VPN 账号。
...
内测拒绝 `--submit-review`、`--notify-testers` 和 `--wait-review`，并校验远端构建确为 `INTERNAL_ONLY`、目标组确为内部组。

显式选择 `--audience external` 时，`--submit-review` 明确允许提交 TestFlight Beta Review；`--notify-testers` 允许启用 Apple 审核通过后的自动通知。
不带通知参数时保留已有通知设置。脚本复用对应的现有测试组，不创建公开链接或扩大测试员名单。
关联前分页查询目标测试组的构建列表，仅在缺少当前构建时追加关联；Apple 不支持直接读取 `Build.betaGroups` 的关联资源，该查询会返回 403。
外测首个构建仍需要 Apple 审核；内测不提审。返回 `WAITING_FOR_BETA_REVIEW` 表示已提交而非已可测试；
使用当前分发范围的状态字段（`internal_state` / `external_state`），仅 `IN_BETA_TESTING` 报告 `testable: true`。可追加 `--wait-review 3600` 有限等待，或之后运行 status。

## 查询与中断恢复

```sh
Apps/iOS/.build/release-venv/bin/python Apps/iOS/scripts/release-testflight.py status --version 0.1.0 --build 8

# 对精确指定的已上传版本继续执行，不再归档/上传
Apps/iOS/.build/release-venv/bin/python Apps/iOS/scripts/release-testflight.py resume \
  --version 0.1.0 --build 8 --audience internal --notes-file Apps/iOS/TEST-NOTES.txt
```

默认等待 Apple 处理最多 1800 秒，可用 `--timeout` 调整。超时可 resume；不要盲目换号再传。
读取 API 对限流和临时服务错误有有限重试；写请求不自动重试，网络错误后先查询远端结果。
远端同版本构建已存在、本地归档已存在、包标识/签名/摘要不符都不会继续上传。内测仅关联 `INTERNAL_ONLY` 包；外测拒绝此类包。
...
`.github/workflows/ios-testflight.yml` 在 main 手动触发，默认 `audience=internal`、`submit_review=false`，同一脚本支持 release 和 resume。
Secrets：`IOS_ASC_KEY_ID`、`IOS_ASC_ISSUER_ID`、`IOS_ASC_PRIVATE_KEY`（`.p8` 全文）。
仓库 Variables：可选的 `IOS_INTERNAL_BETA_GROUP_ID`（默认使用配置中的内测组）、外测用 `IOS_BETA_GROUP_ID`、团队确认后的 `IOS_USES_NON_EXEMPT_ENCRYPTION`（`true`/`false`）；
