...
enum class Phase(val title: String) {
    ...
    /** The engine is working on the user's behalf: show progress and keep the connect button on hold. */
    val transitional get() = this in setOf(CONNECTING, RECOVERING, STOPPING)
}

/** `blocks` persists a pause that only a manual connect clears; the message is the terminal text shown to the user. */
