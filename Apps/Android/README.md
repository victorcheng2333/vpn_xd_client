...
4. 「连接质量」查看当前时长、TLS/DTLS、包计数、最近24小时恢复记录；详细事件最多显示64条，系统分享不包含账号、服务器或凭据。包计数只在此页可见且隧道已连接时每5秒采样，返回此页立即请求刷新；后台、其他页及连接恢复期间停止采样。详细事件和分享均注明最近采样时间，缓存计数不代表最终流量，采样与连接恢复逻辑相互独立。
