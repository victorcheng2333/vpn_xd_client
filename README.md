...
之后应用通过 `sudo -n xd-vpn-helper connect …` 启动 openconnect（前台子进程，实时读它的输出判断状态），`sudo -n xd-vpn-helper disconnect` 发 SIGINT 结束。
...
├── Core/
│   ├── VPNManager.swift         状态机：连接 / 断开 / 自动重连 / 日志 / 登录项
